...
$ingredienteController = new Controllers\ingredienteController();
$comentarioController = new Controllers\comentarioController();
$edamamController = new Controllers\EdamamController();

try {
    // Obtener detalles de la receta local
    $receta = $ingredienteController->obtenerDetalles($id);
    ...
    $ingredientes = $ingredienteController->obtenerIngredientes($id);

    // Obtener comentarios
    $comentarios = $comentarioController->listarComentarios($id);

    // Buscar recetas relacionadas en Edamam
    $recetasExternas = $edamamController->buscarRecetas($receta['titulo'], 0, 5);
    ...
    <main>
        <div class="recipe-header">
            ...
            <div class="recipe-details">
                ...
                <p>Tiempo de preparación: <?= htmlspecialchars($receta['tiempo_preparacion']) ?> minutos</p>
                <p>Por: <?= htmlspecialchars($receta['autor']) ?></p>
            </div>
            <?php if ($receta['imagen']): ?>
                <div class="recipe-image-container">
                    <img class="recipe-image" src="../../src/uploads/<?= htmlspecialchars($receta['imagen']) ?>" alt="<?= htmlspecialchars($receta['titulo']) ?>">
                </div>
            <?php endif; ?>
            <!-- Botón Editar Receta -->
            <?php if (isset($_SESSION['user']) && $_SESSION['user']['google_id'] === $receta['usuario_id']): ?>
                <a href="editar_receta.php?id=<?= htmlspecialchars($id) ?>" class="edit-recipe-button">Editar Receta</a>
            <?php endif; ?>
        </div>
        ...
        <section class="ingredients-section">
            ...
            <ul>
                <?php foreach ($ingredientes as $ingrediente): ?>
                    <li><?= htmlspecialchars($ingrediente['cantidad']) ?> de <?= htmlspecialchars($ingrediente['nombre']) ?></li>
                <?php endforeach; ?>
            </ul>
        </section>
        ...
        <section class="comments-section">
            ...
            <?php foreach ($comentarios as $comentario): ?>
                <div class="comentario">
                    <p><strong><?= htmlspecialchars($comentario['nombre']) ?></strong> (<?= htmlspecialchars($comentario['calificacion']) ?>/5)</p>
                    <p><?= nl2br(htmlspecialchars($comentario['comentario'])) ?></p>
                    <p><em><?= htmlspecialchars($comentario['creado_en']) ?></em></p>
                    <?php if (isset($_SESSION['user']) && $_SESSION['user']['google_id'] === $comentario['usuario_id']): ?>
                        <a href="eliminar_comentario.php?id=<?= htmlspecialchars($comentario['id']) ?>&receta_id=<?= htmlspecialchars($id) ?>"
