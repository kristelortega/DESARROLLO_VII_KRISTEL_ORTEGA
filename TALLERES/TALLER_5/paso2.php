...
function productoMayorValor($inv) {
    ...
    foreach ($inv as $producto => $info) {
        ...
        if ($valorTotal > $mayorValor) {
            ...
            $valorProducto = $valorTotal;
        }
    }

    return ["producto" => $productoMayor, "valor" => $valorProducto];
}

// 9. Mostrar el producto con mayor valor total en inventario y su valor
$resultado = productoMayorValor($inventario);
echo "\nProducto con mayor valor total en inventario: " . $resultado['producto'] . " (Valor total: $" . $resultado['valor'] . ")\n";
