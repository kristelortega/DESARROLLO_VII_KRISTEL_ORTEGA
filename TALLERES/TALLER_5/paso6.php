...
function analizarCrecimientoVentas($ventas) {
    ...
    return [
        "crecimientos" => $crecimientoPorProducto,
        "maxCrecimiento" => $maxCrecimiento,
        "productoMaxCrecimiento" => $productoMaxCrecimiento,
        "regionMaxCrecimiento" => $regionMaxCrecimiento
    ];
}

// Analizar el crecimiento de ventas
$resultadosCrecimiento = analizarCrecimientoVentas($ventas);

echo "\nCrecimiento de ventas (%):\n";
foreach ($resultadosCrecimiento['crecimientos'] as $clave => $crecimiento) {
    echo "$clave: " . number_format($crecimiento, 2) . "%\n";
}

echo "\nProducto con mayor crecimiento: {$resultadosCrecimiento['productoMaxCrecimiento']} en {$resultadosCrecimiento['regionMaxCrecimiento']} (Crecimiento: " . number_format($resultadosCrecimiento['maxCrecimiento'], 2) . "%)\n";
