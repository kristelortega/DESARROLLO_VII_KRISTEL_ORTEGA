...
function buscarLibros($biblioteca, $termino) {
    $resultado = [];
    $termino = strtolower($termino); // Hacer la búsqueda insensible a mayúsculas/minúsculas

    foreach ($biblioteca as $libro) {
        if (strpos(strtolower($libro['titulo']), $termino) !== false || 
...
// 11. Crear una función que genere un reporte de la biblioteca
    function generarReporteBiblioteca($biblioteca) {
        ...
        arsort($librosPorAutor); // Ordenar para que el autor con más libros esté primero
